...
# gtfAttributeParse
##-----------------------------------------------------##
source("../../A_modules/0_Rfunction_gtfAttributeParser.R")
...
#name <- "chicken_Ren"
#gtf <- "/home/fabien/References/GalGal6/1_LNCextendedEns101.gtf.gz" 



txt_file <- paste0(name,"_classes_feelncclassifier.txt")
log_file <- paste0(name,"_feelncclassifier.log")
...
gtf_genes <- data.frame(gtf_genes, gtfAttributeParser(gtf_genes), stringsAsFactors = FALSE)

gtf_transcripts <- gtf[gtf$feature == "transcript", ]
gtf_transcripts <- data.frame(gtf_transcripts, gtfAttributeParser(gtf_transcripts), stringsAsFactors = FALSE)


##-----------------------------------------------------##
# Classified genes
##-----------------------------------------------------##
feelnc_results_lncRNArelativeTo_mRNA <- read.table(txt_file,
...
# Unclassified genes
##-----------------------------------------------------##
feelnc_unclassified_lnc_relativeTo_pcg <- scan(log_file,
                                               what = "character", quiet = T)
pos_firsttpID <- grep("ENS*", feelnc_unclassified_lnc_relativeTo_pcg, perl = T)[1] # theorical position of the first transcript id in the vector : 51. To check if re-used
...
write.table(annotation_gene, paste0(name,"_lncConfiguration_feelncclassifier.tsv")
            , quote = F, sep = "\t",
