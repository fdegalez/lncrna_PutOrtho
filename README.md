...
- [Output Overview](#output-overview)
  - [Description of main output files](#description-of-main-output-files)
  - [Synteny-based orthology (Method 1)](#synteny-based-orthology-method-1)
  - [FEELnc genomic configuration orthology (Method 2)](#feelnc-genomic-configuration-orthology-method-2)
  - [Genome alignment orthology (Method 3)](#genome-alignment-orthology-method-3)
- [Typical Workflow](#typical-workflow)
- [Notes and Limitations](#notes-and-limitations)
- [Comments / Questions / Bugs](#comments--questions--bugs)
- [Authors](#authors)
- [License](#license)
...
A complete **test dataset including both input files and expected outputs** is available on Zenodo:

https://zenodo.org/records/19048041

This dataset allows users to reproduce the pipeline execution and verify that the installation and configuration are working properly.  
It contains example GTF annotations, configuration files, and example outputs generated by the different modules of the pipeline.
...
## Description of main output files
### Synteny-based orthology (Method 1)

Main output files:

    3_synteny/results_syntenyMerged/[species]_syntenyMerged.tsv

Each row corresponds to a **lncRNA from the reference species**.

Important columns include:

- `lncRNA.<speciesRef>` : identifier of the lncRNA in the reference species
- `lncRNA.<speciesAlt>` : candidate orthologous lncRNAs detected in other species
- `orthology_type` : type of orthology relationship detected (one-to-one, one-to-many, etc.)
- `nbTotalIndication` : number of species supporting the orthology relationship
- `profilIndication` : summary of orthology types detected across species

This file summarizes orthology predictions inferred from **conserved genomic synteny between protein-coding genes**.

### FEELnc genomic configuration orthology (Method 2)

Main output files:

    4_FEELnc/results_orthoFeelnc_merged/[species]_feelncMerged.tsv

Each row corresponds to a **lncRNA gene** whose genomic configuration relative to nearby protein-coding genes is conserved across species.

Important information includes:

- `lncRNA.<speciesRef>` : lncRNA identifier
- `lncRNA.<speciesAlt>` : FEELnc genomic configuration class (antisense, divergent, intergenic, etc.)
- `orthology_type` : orthology type inferred from conserved configuration
- `nbTotalIndication` : number of species supporting the orthology relationship
- `profilIndication` : summary of orthology types detected across species

This method detects orthologous relationships based on **conservation of genomic context relative to neighboring protein-coding genes**.


### Genome alignment orthology (Method 3)

Main output files:

    5_compara/results_isMatching/[species]_isMatching.tsv

Each row corresponds to a **lncRNA genomic region found in the Ensembl Compara multiple genome alignment**.

Important columns include:

- `lncRNA` : identifier of the queried lncRNA
- `nbSpeciesMatch_tot` : number of species with an aligned genomic region (MP-65amniotes)
- `SpeciesMatch_tot` : list of species where the region is aligned (MP-65amniotes)
- `<species>_isMatching` : binary indicator showing whether an aligned region was detected in each species
- `nbSpeciesMatch_set` : Number of species in our inital dataset where a match was found

This method provides evidence of **sequence conservation of lncRNA genomic loci through multi-species genome alignments**.
