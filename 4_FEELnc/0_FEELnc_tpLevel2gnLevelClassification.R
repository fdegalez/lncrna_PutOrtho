...
for (name in name.list){
  ...
  gtf.path <- config$pathToGTF[config$completeName == name]
  ...
  gtf$transcript_id <- pbsapply(gtf$attribute, extract_field, "transcript_id")
  ...
  txt.path <- list.files("results", pattern = name, full.names = T)
  txt.path <- txt.path[grep("_classes_feelncclassifier.txt", txt.path)]
  
  
  log.path <- list.files("results/logs", pattern = name, full.names = T)
  ...
  write.table(annotation_gene, paste0("results_geneLevel/", name,"_lncConfiguration_feelncclassifier.tsv")
              , quote = F, sep = "\t",
